...
## Current Focus
- Debugging persistent Django admin `RecursionError: maximum recursion depth exceeded` related to the `profiles` app and its models (notably `StatementFile`/`UploadedFile`).
- Ensuring a truly clean database state for migrations.

## Debugging Steps & Tests Performed
- Disabled all custom admin templates and admin.py files.
- Removed all custom apps from `INSTALLED_APPS` except Django built-ins; error only returned when `profiles` was re-added.
- Isolated error to presence of `StatementFile`/`UploadedFile` model, even as an empty class.
- Removed all ManyToMany and ForeignKey relationships between `UploadedFile` and `Transaction`.
- Commented out all admin, forms, and template tag references to `UploadedFile`.
- Cleared Python bytecode caches and tested in both the main and a clean virtual environment.
- Cleaned up Django content types and permissions for old model names.
- Attempted to comment out the model entirely, but encountered import errors due to references in admin, forms, and views.
- Commented out all references to `UploadedFile` in the codebase to allow the app to start.
- Restored the `statement_upload_to_uuid` function for migration compatibility.
- Ran `makemigrations` and `migrate` after each change.
- Disabled all custom admin templates and admin.py files.
- Attempted to drop and recreate the database using Docker and Postgres commands; issues with missing roles and database not being empty.
- Discovered `.env.dev` was controlling the `DATABASE_URL` and sometimes overriding `settings.py`.
- Created a new database (`ledgerflow_fresh`) and pointed Django to it, but migrations still failed due to pre-existing tables.
- Wrote and ran a Python script to drop all tables in the `public` schema; enhanced to drop all tables, views, and sequences.
- Confirmed all objects dropped; migrations still failed due to lingering objects (now resolved).
- Deleted all `profiles`-related files to fully isolate the error source.
- Rebooted the system and re-tested; error persists.
