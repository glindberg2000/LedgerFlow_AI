...
class BusinessProfileAdmin(admin.ModelAdmin):
    ...
    list_display = ("client_id", "company_name", "business_type")
    ...
    fieldsets = (
        ...
        (
            ...
            {
                "fields": ("client_id",),
                "description": "System identifier (read-only)",
            },
        ),
    )
    ...
    try:
        ...
        for transaction in queryset:
            ...
            update_fields = get_update_fields_from_response(
                agent,
                response,
                "payee" if "payee" in agent.name.lower() else "classification",
            )
            logger.info(
    ...
    def batch_payee_lookup(self, request, queryset):
        ...
        for client_id, data in client_transactions.items():
            with db_transaction.atomic():
                ...
                task.task_metadata["transaction_ids"] = data["transaction_ids"]
                task.save()
                messages.success(
                    request,
...
class StatementFileAdmin(admin.ModelAdmin):
    ...
    def batch_upload_view(self, request):
        ...
        )  # Add admin context including available_apps
